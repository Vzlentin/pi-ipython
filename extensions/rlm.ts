...
export default function rlmExtension(pi: ExtensionAPI) {
	...
	pi.registerTool({
		name: "ipython",
		label: "IPython",
		description: `Execute Python in a persistent IPython kernel. Reuse variables, functions, datasets, and intermediate results across calls; no RLM child calls or skill invocation are required for ordinary Python work. Supports top-level await and native IPython magics. State lasts only for the kernel process; resets and reloads lose it. The kernel runs with local user permissions, including filesystem and network access; it is not sandboxed. The rlm object is already bound; do not import over it. Child calls: h = await rlm.spawn("task", context="text"); [r] = await rlm.gather([h]); use r.text only after checking r.status == "ok", otherwise inspect r.status and r.error. spawn, gather and release are coroutines; context must be a str or None; children have no tools or filesystem, so put everything they need in context. Also await rlm.release(handles) and await rlm.final(value). Gather delivers each handle once and recovers committed results after transport loss. Child calls are limited to ${MAX_CHILDREN_RUNNING} concurrent/${MAX_LIVE_HANDLES} live handles, ${formatSize(MAX_CHILD_REQUEST_BYTES)} input, ${formatSize(MAX_CHILD_TEXT_BYTES)} returned text, and a 5-minute deadline. Output is truncated to ${DEFAULT_MAX_LINES} lines or ${formatSize(DEFAULT_MAX_BYTES)}; full truncated output is saved to a temporary file. Runaway cells exceeding ${formatSize(OUTPUT_CAPTURE_LIMIT_BYTES)} of output are stopped and reset the kernel.`,
		promptSnippet: "Persistent Python workspace for computation, data analysis, and optional focused RLM child calls",
		promptGuidelines: [
